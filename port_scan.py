...
from Utils.socks_resolver import get_addr_info
from Utils.server_scanner import discover_by_port
...
def target_to_host(target):
    """
    攻击目标，比如  http://127.0.0.1 转换为 127.0.0.1
    Args:
        target: 目标
...
def target_type(target):
    ...
    Returns:
        目标类型 (SINGLE_IPv4, SINGLE_IPv6, RANGE_IPv4, DOMAIN, HTTP, CIDR_IPv4, UNKNOWN)
    """
    if isIP(target):
        return "SINGLE_IPv4"
    elif isIP6(target):
...
def stealth(host, port, timeout_sec, time_sleep, socks_proxy, scan_id,
    ...
    try:
        if socks_proxy is not None:
            ...
            if '@' in socks_proxy:
                ...
                socket.getaddrinfo = get_addr_info
            else:
            ...
            if (stealth_scan_resp.getlayer(TCP).flags == 0x12):
                ...
                data = json.dumps(
                    {'HOST': host, 'USERNAME': '', 'PASSWORD': '', 'PORT': port, 'TYPE': 'port_scan',
                     'DESCRIPTION': "port/type".format(str(port) + service_name, "STEALTH"),
                     'TIME': now(),
...
def __udp(host, port, timeout_sec, language, time_sleep, thread_tmp_filename, socks_proxy, scan_id,
          scan_cmd, stealth_flag):
    try:
        if socks_proxy is not None:
            socks_version = socks.SOCKS5 if socks_proxy.startswith(
        ...
        try:
            ...
            service_name = ""
        print("port_found".format(host, str(port) + service_name, "UDP"),
              "a", {'HOST': host, 'USERNAME': '', 'PASSWORD': '', 'PORT': port, 'TYPE': 'port_scan',
                    'DESCRIPTION': "port/type".format(str(port) + service_name, "UDP"),
        ...
        try:
            if filter_port(host, port):
                print("port_found".format(host, str(port) + service_name, "TCP_CONNECT"))
                data = json.dumps({'HOST': host, 'USERNAME': '', 'PASSWORD': '', 'PORT': port, 'TYPE': 'port_scan',
                                   'DESCRIPTION': "port/type".format(str(port) + service_name, "TCP_CONNECT"),
...
def connect(host, port, timeout_sec, time_sleep, thread_tmp_filename, socks_proxy, scan_id,
            scan_cmd, stealth_flag):
    try:
        if socks_proxy is not None:
            socks_version = socks.SOCKS5 if socks_proxy.startswith(
...
def start(target, users, passwds, ports, timeout_sec, thread_number, num, total,  time_sleep,
    ...
    if target_type(target) != 'SINGLE_IPv4' or target_type(target) != 'DOMAIN' or target_type(
        ...
        for port in ports:
            t = threading.Thread(target=stealth if stealth_flag else (__udp if udp_flag else connect),
                                 args=(target, int(port), timeout_sec, time_sleep,
                                       socks_proxy, scan_id, scan_cmd, stealth_flag))
            threads.append(t)
            t.start()
            time.sleep(time_sleep)
            trying += 1
            if verbose_level > 3:
                print("trying_message".format(trying, total_req, num, total, target, port, 'port_scan'))
            while 1:
                try:
        ...
        print("input_target_error".format('port_scan', target))
