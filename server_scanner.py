...
ports_services_and_condition = {
    "http": ["HTTP", ["OK", "CREATED", "No Content", "Moved Permanently", "Found", "Not Modified", "Permanent Redirect",
                      ...
                      "Internal Server Error", "Bad Gateway", "Service Unavailable", "Gateway Timeout", "444 No Response"
                      ],
             ],
...
ports_services_regex = {
    "http": ["HTTP\/[\d.]+\s+[\d]+", ],  # 匹配http成功响应 HTTP/1.0 200 OK, etc.
    "ftp": ["FTP\/[\d.]+\s+[\d]+"],  # 同上
...
def discovery(target, ports=None, timeout=3, thread_number=1000, send_data=None, time_sleep=0, socks_proxy=None):
    """
    发现端口运行的服务,并且探测修改过默认端口的真实服务名

    Args:
        target: 扫描目标
        ports: 端口列表,如果为None则测试默认的1000个公共端口
        timeout: 超时时间
        thread_number: 扫描线程数
        send_data: 要发送的套接字数据, 如果为None则发送默认的 b"ABC\x00\r\n" * 10
        time_sleep: 请求睡眠间隔
        socks_proxy: socks 代理
    Returns:
        服务和端口字典
    ...
    if not send_data:
        send_data = b"ABC\x00\r\n" * 10
    if not ports:
        from port_scan import extra_requirements_dict as port_scanner_default_ports
